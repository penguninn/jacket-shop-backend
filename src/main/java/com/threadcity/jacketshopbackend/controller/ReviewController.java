...
public class ReviewController {
    ...
    public ReviewResponse update(@PathVariable Long id, @Valid @RequestBody ReviewUpdateRequest request) {
        return reviewService.updateReviewById(request, id);
    }
    ...
    @GetMapping("/product/{productId}")
    public PageResponse<?> getByProduct(@PathVariable Long productId,
                                        @RequestParam(defaultValue = "0") int page,
                                        @RequestParam(defaultValue = "10") int size) {
        ReviewFilterRequest filter = ReviewFilterRequest.builder()
                ...
                .sortDir("DESC")
                .build();
        return reviewService.getAllReviews(filter);
    }

    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    public PageResponse<?> getAll(ReviewFilterRequest request) {
        return reviewService.getAllReviews(request);
    }
}